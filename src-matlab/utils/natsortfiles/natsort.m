...
% Alphanumeric / Natural-Order sort a cell array of 1xN char.
%
% (c) 2012-2020 Stephen Cobeldick
%
% Alphanumeric sort a cell array of strings: sorts by character order and
% also by the values of any number substrings. By default matches integer
% substrings and performs a case-insensitive ascending sort, optional
% inputs allow the user to select other number formats, sort order, etc.
%
%%% Example:
% >> X = {'x2', 'x10', 'x1'};
...
% [Y,ndx,dbg] = natsort(X,...)
%
% To sort filenames, foldernames, or filepaths use NATSORTFILES (FEX 47434).
...
%
%%% Inputs (*==default):
% X   = CellArrayOfCharRowVectors, to be sorted into alphanumeric order.
...
%
%%% Outputs:
% Y   = CellArrayOfCharRowVectors, <X> sorted into alphanumeric order.
...
%
assert(iscell(X),...
	'SC:natsort:X:NotCellArray',...
	'First input <X> must be a cell array.')
tmp = cellfun('isclass',X,'char') & cellfun('size',X,1)<2 & cellfun('ndims',X)<3;
assert(all(tmp(:)),...
	'SC:natsort:X:ContentNotCharVectors',...
	'First input <X> must be a cell array of char row vectors (1xN char).')
%
...
assert(all(tmp(:)),...
	'SC:natsort:options:NotCharVector',...
	'All optional arguments must be char row vectors (1xN char).')
% Character case:
...
nsAssert(1,varargin,chx,'CharNumOrder', 'char<->num')
nsAssert(1,varargin,nax,'NanNumOrder',  'NaN<->num')
nsAssert(1,varargin,sfx,'sscanfFormat', 'SSCANF format')
nsAssert(0,varargin,~(ccx|sdx|chx|nax|sfx),'UnusedOption')
%
% SSCANF format:
if nnz(sfx)
	fmt = varargin{sfx};
...
if nmx
	...
	assert(numel(vec)==numel(tmp),...
		'SC:natsort:sscanfTooManyValues',...
